...
update_git_plugin() {
    ...
    if [[ ! -d "${plugin_path}" ]]
    then
        cout error "directory «${plugin_path}» doesn't exists."
    fi

    if [[ -d "${plugin_path}/.git" ]]
    then
        pushd "${plugin_path}" &> /dev/null || cout error "Can't move to «${plugin_path}»"
        git fetch --all
        git pull
    else
        cout error "«${plugin_path}» not a git repo."
    fi
}

#handle_plugins $git_repo $plugin_path $plugin_name $enabled (Y/N)
handle_zsh_plugin() {
    missing_argument_validation 4 "${1}" "${2}" "${3}" "${4}"
