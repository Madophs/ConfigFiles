...
function! g:SaveSession()
    ...
    let g:mds_session_file = system("(REPONAME=$(git -C '" .. getcwd() .. "' rev-parse --show-toplevel 2> /dev/null) && echo $REPONAME | awk -F '/' '{print \"git_\"$NF\".vim\"}') || echo " .. getcwd() .. " | awk -F '/' '{print $NF\".vim\"}'")
